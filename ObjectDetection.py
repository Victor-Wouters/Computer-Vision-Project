...
def main(input_video_file: str, output_video_file: str) -> None:
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            if between(cap, 2000, 8000):  # For example, between 2s and 8s
                ...
                # Loop over the frame by regions to compute region histograms and their comparison to the object histogram
                for y in range(0, hsv_image.shape[0], region_size):
                    for x in range(0, hsv_image.shape[1], region_size):
                        ...
                        # Use a proper histogram comparison method, or adjust your MSE calculation method here
                        # For demonstration, let's assume we use histogram correlation as a similarity measure
                        similarity = histogram_similarity(object_histogram, region_histogram)

                        # Store the similarity (or inversely, the mse) for later normalization
                        mse_values.append(1 - similarity)  # Assuming similarity inversely relates to MSE

                # Normalize MSE values to span the 0-255 range
                mse_values = np.array(mse_values)
                min_mse, max_mse = mse_values.min(), mse_values.max()
                mse_values_normalized = (mse_values - min_mse) / (max_mse - min_mse) * 255

                # Update the grayscale image with normalized intensity values
                index = 0
                for y in range(0, hsv_image.shape[0], region_size):
                    for x in range(0, hsv_image.shape[1], region_size):
                        intensity = 255 - mse_values_normalized[index]  # Inverting intensity: lower MSE = higher intensity
...
def compute_histogram(image, mask=None):
    ...
    histogram = cv2.calcHist([hsv_image], [0, 1], mask, [180, 256], [0, 180, 0, 256])
